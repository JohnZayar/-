document.addEventListener("DOMContentLoaded", () => {
    ...
    // YBS Local JSON ကို ဖတ်မည် (VPN မလိုပါ)
    ...
    // GPS ခလုတ် နှိပ်သည့်အခါ (မူလအတိုင်း အလုပ်လုပ်ရန်)
        ...
        gpsBtn.addEventListener("click", () => {
            if (navigator.geolocation) {
                navigator.geolocation.getCurrentPosition(position => {
                    ...
                    document.getElementById("start-location").value = `လက်ရှိတည်နေရာ (${lat.toFixed(4)}, ${lon.toFixed(4)})`;
                    calculateFareAndBuses();
                }, () => {
                    alert("တည်နေရာရယူ၍ မရပါ။");
                });
            } else {
                alert("GPS မပံ့ပိုးပါ။");
            }
        });
    }

    // နေရာများ ရွေးချယ်သည့်အခါ တွက်ချက်ရန်
    const startInput = document.getElementById("start-location");
    const endInput = document.getElementById("end-location");

    if(startInput && endInput) {
        startInput.addEventListener("input", calculateFareAndBuses);
        endInput.addEventListener("input", calculateFareAndBuses);
    }

    function calculateFareAndBuses() {
        const start = startInput.value.trim();
        const end = endInput.value.trim();

        // ဥပမာ တက္ကစီခ တွက်ချက်မှုပြသခြင်း (မူလအတိုင်း)
        document.getElementById("total-fare").innerText = "7,600";
        document.getElementById("base-fare").innerText = "2,500 Ks";
        document.getElementById("km-fare").innerText = "4,800 Ks";
        document.getElementById("waiting-fare").innerText = "300 Ks";

        if(start && end) {
            findMatchingBuses(start, end);
        }
    }

    function findMatchingBuses(start, end) {
